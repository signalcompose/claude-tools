...
Dev Cycle とその構成スキル（`sprint-impl`, `audit-compliance`, `shipping-pr`, `retrospective`）は、**ソースコードの作成・変更・コミット・プッシュ・PR作成を自律的に実行**します。これは Claude Code の通常の対話モードとは異なり、ユーザーの確認なしに多くの操作を連続実行する設計です。

本ガイドで推奨するパーミッション設定は、この自律実行を実現するために必要なものですが、**設定の変更と使用はユーザーの責任**で行ってください。パーミッションを変更する前に、各設定項目の意味と影響を理解した上で判断してください。
...
スプリントは以下のフェーズで構成されています（主要フェーズ + 中間フェーズ）:
...
| 6.1 | インクリメンタルコミット | エージェントごとの中間コミット |
| 6.5 | Serena シンボル検証 | オプション（Serena MCP利用時） |
| 7 | 統合検証 | tsc, vitest, eslint |
| 7.5 | コンプライアンスゲート | DDD/TDD/DRY/ISSUE 事前チェック |
| 8 | カバレッジ確認 | 閾値（デフォルト80%）チェック |
| 9 | サマリーレポート | - |
| 10 | Serena メモリ保存 | オプション（Serena MCP利用時） |
| 11 | 自動シップ | Dev Cycle外で実行時のみ（Dev Cycle内ではスキップ） |
...
**⚠️ 変更操作（Sandbox 有効時は allow、無効時は ask 推奨）**
...
**理由**: これらは開発に不可欠な操作です。**Sandbox モードが有効な場合**、Bash コマンドはサンドボックス内で実行されるため、`allow` に配置しても安全です（推奨設定）。**Sandbox モードが無効な場合**、`ask` に配置してユーザー確認を求めることを推奨します。
...
**注意**: `setup-dev-env` スキル（Check 7）は `sandbox.enabled` と `autoAllowBashIfSandboxed` の両方をチェックします。これらが未設定の場合、WARN として報告されます。
...
**重要**: 以下のパーミッション設定は、Dev Cycle の自律実行に必要な権限を列挙しています。Dev Cycle はスプリント実装からPR作成までを自動で行うため、通常の対話モードより広い権限が必要です。これらの設定を適用するかどうかは **ユーザー自身の判断と責任** で行ってください。設定内容を理解した上で、ご自身のプロジェクトに合った権限レベルを選択してください。

### 必要なパーミッション一覧

#### `.claude/settings.local.json`（ユーザーローカル設定）

以下の例は **Sandbox モード有効** を前提とした設定です。Sandbox により Bash コマンドはサンドボックス内で実行されるため、git 操作を `allow` に含めても安全です。Sandbox を無効にする場合は、git 書き込み操作（`git add`, `git commit`, `git push`）を `allow` から外し、デフォルトの `ask` で運用してください。
