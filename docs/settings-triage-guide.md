...
> **表記**: 山括弧で囲まれた識別子 (`<plugin>`, `<user>`, `<project>` 等) は、文脈に応じて置換すべき変数部分を示す。
> **用語**: 本ガイドでは「棚卸し」を主用語とする。同義で「トリアージ」と呼ぶ場合もあるが指すプロセスは同じ。
...
- `settings.local.json` の `allow` 項目が **50 個以上** (経験的に、この規模を超えると個人 / 共有の混在や陳腐化した pattern が発生しやすい)
...
for f in .claude/settings.json .claude/settings.local.json; do
  [ -f "$f" ] && jq -r '"\(input_filename): allow=\(.permissions.allow|length), ask=\(.permissions.ask|length), deny=\(.permissions.deny|length)"' "$f"
done
```

allow 数が 50 を超えていたら次ステップへ。
...
- 指摘が出たら修正 → 再レビュー (収束までの iteration 数は「参考実績」セクション参照)
...
> ⚠️ **`--force-with-lease` は上記 deny でブロックされない**。`Bash(git push --force :*)` の space-as-word-boundary 挙動 (後述「Pattern Syntax Notes」参照) により `--force-with-lease` は別トークン扱い。`--force-with-lease` を禁止する場合は `Bash(git push --force-with-lease :*)` を個別追加するか、`ask` に置くこと。

(シークレット読み取り: `Read` deny は Bash 経由読み取りを防げない。詳細は上記「シークレット読み取り防止」セクションの ⚠️ 注意を参照。)
...
## built-in `fewer-permission-prompts` スキルとの関係

Claude Code には `fewer-permission-prompts` という built-in スキルがある。transcript を scan して頻出の read-only 操作を `.claude/settings.json` に allow として追加する。
...
claude-tools での棚卸し実施結果 (2026-04-17 時点、関連PR #191/#192 マージ後に現ファイルへ反映):
...
関連 PR:
- [signalcompose/claude-tools#191](https://github.com/signalcompose/claude-tools/pull/191): `code` プラグイン同梱 settings に最小 deny リスト追加
- [signalcompose/claude-tools#192](https://github.com/signalcompose/claude-tools/pull/192): プロジェクト settings の棚卸しと CLAUDE.md 整合強化
