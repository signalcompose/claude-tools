...
| 内部の状態 | 出力 | exit |
|---|---|---|
| ジョブが `completed` になった | `DONE` | 0 |
| ジョブが `failed`/`cancelled` で終わった | `FAILED` | 4 |
| ジョブが state から消えた（該当なしと確認できた） | `DONE` | 0 |
| state ファイルが読めず該当も見つからない | `WARN` ×3 → `ERROR ジョブの生死は不明。DONE ではない。` | 1 |

最後の行が要点。上流は `state.json` を temp+rename ではなく直接書き換えるため、
**書き込み中のポーリングは空・不完全なファイルを見ることがある**。これを
「ジョブが無くなった」と読むと監視が DONE を誤報する。読めなかったファイルが
1つでもあれば「該当なし」と断定しない。
...
⚠️ `stat -f %m` が返すのは **Unix epoch 秒でタイムゾーンに依存しない**ので、
ログ行の `...Z` (UTC) との比較にズレは生じない。ズレるのは `ls -l` や
`stat -f %Sm` のような**整形済みの表示**で、こちらはローカル時刻になる
（ログ行が `11:17Z` のファイルを `ls` が `20:17` と表示する）。
比較は必ず epoch 同士で行い、整形済みの表示を突き合わせない。
