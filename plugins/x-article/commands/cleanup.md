...
```bash
# macOS: trash コマンドが利用可能な場合（推奨）
trash ./x-article/

# trash コマンドがない場合のフォールバック
# open -R ./x-article/ でFinderで開いてもらう案内、
# または rm -rf ./x-article/ を提案する
```

`trash` コマンドの有無を確認してから実行すること:

```bash
if command -v trash &>/dev/null; then
  trash ./x-article/
else
  echo "trash コマンドが見つかりません"
  echo "brew install trash でインストールするか、手動で削除してください"
fi
```
