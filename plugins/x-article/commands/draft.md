...
**保存先**: デフォルトは `~/Desktop/`。ユーザーに保存先を確認し、指定がなければ `~/Desktop/` に保存する。
Write ツールには絶対パスが必要なため `~` を展開すること（例: `/Users/username/Desktop/`）。
**重要**: プロジェクトのカレントディレクトリ（`cwd`）には保存しないこと。ドラフトはプロジェクト外のパーソナルフォルダに置くべきである。
