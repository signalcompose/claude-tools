...
- Auto mode's classifier auto-approves normal `Bash` calls (via `autoAllowBashIfSandboxed: true`) *inside* the sandbox, including plugin cache path bash invocations such as `bash ${CLAUDE_PLUGIN_ROOT}/scripts/*.sh` (both bare and compound forms like `PLUGIN_ROOT=... && bash $PLUGIN_ROOT/scripts/*.sh`). No additional auto-approve hook is needed.
