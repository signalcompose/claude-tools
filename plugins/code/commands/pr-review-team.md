...
4. **統合・修正** — レビュー結果 + CI 結果を fixer に一括送信（Codex を利用可能なら優先し、利用不可時は従来どおり Sonnet にフォールバック）
5. **反復ループ** — Critical/Important = 0 かつセキュリティ全合格まで（最大3回）。ラウンド2以降は code-reviewer、直前ラウンドで問題を報告した reviewer、および selector の新規追加分のみ再起動
