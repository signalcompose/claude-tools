...
MANDATORY (no text output, no user confirmation):
  1. Bash: if [ -f .claude/dev-cycle.state.json ]; then jq --arg s \"${NEXT_STAGE}\" '.stage=\$s' .claude/dev-cycle.state.json > .claude/dev-cycle.state.json.tmp && mv .claude/dev-cycle.state.json.tmp .claude/dev-cycle.state.json || rm -f .claude/dev-cycle.state.json.tmp; else printf '{\"stage\":\"%s\"}' \"${NEXT_STAGE}\" > .claude/dev-cycle.state.json; fi
